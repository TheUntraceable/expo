...
public struct NotificationContentRecord: Record {
  ...
  public init(from request: UNNotificationRequest) {
    ...
    if let sound = content.sound {
      ...
      print("[NotificationRecord] Creating Either with sound value: \(soundValue)")
      self.sound = Either<Bool, String>(soundValue)
      print("[NotificationRecord] Created Either: \(String(describing: self.sound))")
    } else {
      print("[NotificationRecord] Sound is nil, not setting")
    }
